...
| Unit tests | 104 passed across 15 files | Tenant boundaries, safe responses, adapter, invitations, jobs, cron, receipts, package review gate, admin ownership/cancellation/key rotation, enrollment, branding URLs, schedule validation, preview routing/data safety, draft onboarding, Workspace/Clientspace resolution and activation prerequisites; database/provider mocked |
...
| Authenticated local admin | Passed | Real Superadmin login, four persisted package cards and package editor; settings save persisted successfully; Devsouq Workspace synced and activated |
...
- Unit tests cover isolated-key scope, exact-name Workspace/Clientspace resolution from an agency key, cross-client mapping rejection, duplicate sync submission, five data checks before publication, provider/SMTP failure handling, owner access, invite deduplication, campaign pagination/final-cursor handling, tenant-bound status, targeted job execution and safe extended contact/tag fields. Only the resolved isolated key is encrypted and stored.
...
- Hostinger SMTP authentication for `info@reliantoutreach.com` passed, and the live invitation email job completed. The app accepts isolated Workspace or Clientspace keys directly and agency keys only after an exact, unique account-name match. The campaign sync accepts Manyreach&apos;s final page when a stale next cursor remains after the total is reached. Devsouq synced successfully from its Workspace connection, became ACTIVE, sent the owner invitation through Hostinger SMTP, and recorded the invitation as accepted. The client preview loaded all Dashboard, Campaigns, Inbox, Prospects, Lists, Senders, Analytics, Team, Usage, Notifications and Settings tabs without errors. Source remains local.
