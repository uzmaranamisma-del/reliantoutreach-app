...
vi.mock("@/features/portal/preview", () => ({ PreviewPortal: () => null }));
import { admin } from "@/lib/access";
import { db } from "@/lib/db";
import Page from "@/app/admin/client-preview/[id]/[[...path]]/page";
...
it("does not read client data when administrator authorization fails", async () => {
  ...
  await expect(
    Page({ params: Promise.resolve({ id: "private-client", path: [] }) }),
...
it("uses saved client data and leaves unavailable metrics empty", async () => {
  ...
  const page = await Page({
    params: Promise.resolve({ id: "draft", path: [] }),
  });
